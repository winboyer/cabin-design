...
def create_docx_from_multiple_folders(parent_directory, output_filename):
    """
    从指定父目录读取多个迭代文件夹，生成包含所有内容的DOCX文档
    ...
    # 查找所有迭代文件夹
    iter_folders = [f for f in os.listdir(parent_directory)
    ...
    # 处理每个迭代文件夹
    for i, iter_folder in enumerate(iter_folders):
        directory_path = os.path.join(parent_directory, iter_folder)
        ...
        # 添加迭代文件夹标题
        folder_title = doc.add_paragraph(f"迭代文件夹: {iter_folder}")
        folder_title.alignment = WD_PARAGRAPH_ALIGNMENT.CENTER
        for run in folder_title.runs:
            run.bold = True
            run.font.size = Pt(16)
            run.font.name = '宋体'
            ...
            if filename.lower().endswith('.txt'):
                ...
                try:
                    ...
                    if content_lines:
                        ...
                        # 如果是第一个文本文件且还未设置主标题，设置为主标题
                        if not main_title_set and i == 0:
                            # 更新主标题
    ...
    # 保存文档
    save_path = os.path.join(parent_directory, output_filename)
    doc.save(save_path) # type: ignore[reportGeneralTypeIssues]
    print(f"已成功生成文档: {save_path}")
